    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          'Operational Data: We keep anonymous totals for API requests, app versions, operating systems, failures, and active SSE connections. These totals carry no account identifier, IP address, or ticker contents. Separately, optional PostHog analytics remain off until you enable them and use only the fixed public-page or desktop events described below.',
          ...
          'Optional PostHog Analytics: The website can send public-page paths without query strings plus verified signup and platform download steps. Website events explicitly disable IP and location enrichment; each event uses a fresh random identifier. The desktop can send app opened, app running, and broad configured-feature categories through our API under a pseudonymous account identifier. Private routes, ticker contents, symbols, teams, feeds, support text, session replay, and unrelated device activity are excluded. Website and desktop choices are separate and both default off.',
        ],
      },
      {
      ...
      },
      {
        ...
        content: [
          'We retain your account information and preferences for as long as your account is active. Optional product-activity daily facts are retained for 90 days. While PostHog analytics are enabled, Scrollr keeps an export mirror of the fixed account-linked desktop event fields. After desktop opt-out it remains only until PostHog accepts the deletion request; account purge removes the mirror immediately and keeps only a minimal deletion tombstone. RSS articles are automatically deleted after 7 days. Yahoo Fantasy data is refreshed on a sync cycle (default: every 120 seconds for active users).',
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          'Scrollr includes no advertising frameworks or data sales. Optional PostHog analytics are separate from crash reports and remain off until you accept them. Public-site analytics exclude private routes and page contents. Desktop analytics use a pseudonymous account identifier and fixed broad events only; they never include ticker contents, symbols, teams, feeds, support text, or unrelated device activity. The independent "Send crash reports" setting controls Sentry.',
        ],
      },
      {
        heading: 'Data Deletion',
        content: [
          'You can delete local preferences with "Reset all settings." Turning off "Share product activity" stops first-party collection and deletes its server-side daily facts and cohort metadata. Website PostHog opt-out stops new capture immediately; its earlier fixed-field observations have no durable browser or account identifier. Desktop PostHog opt-out stops new capture and requests deletion of the account pseudonym and prior events; PostHog processes that deletion asynchronously. Account deletion removes the local desktop event mirror immediately and retains only a deletion tombstone until that request is accepted.',
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          'We do not use advertising cookies or tracking pixels. Optional PostHog analytics remain off until you choose to enable them. The consent choice is stored in browser local storage; PostHog identity persistence is memory-only. Each event uses a fresh random identifier; a pending sign-in keeps one random identifier in session storage only until signup verification. PostHog receives only the fields described in our Privacy Policy and no private-page URLs, query strings, page contents, session replays, IP enrichment, or advertising-profile data.',
        ],
      },
      {
        ...
        content: [
          ...
          "Authentication, preferences, and widget data are sent only to Scrollr's API servers to authenticate requests and retrieve your personalized data. If you separately enable PostHog app analytics, Scrollr's API sends a pseudonymous identifier and fixed content-free activity categories to PostHog; ticker contents are never included.",
        ],
      },
      {
        ...
        content: [
          ...
          'Analytics export mirror: While PostHog analytics are enabled, Scrollr keeps the fixed desktop event name, broad feature category, app version, timestamp, and delivery status so your account export includes the account-linked events sent to PostHog. On desktop opt-out, this mirror remains only until PostHog accepts the deletion request. Account purge removes it immediately and keeps only the minimal deletion tombstone needed to retry the vendor request.',
        ],
      },
      {
        ...
        content: [
          ...
          'Website analytics: Use "Analytics privacy choices" in the site footer to allow or decline PostHog analytics. Declining stops capture, clears in-memory SDK identity state, and records your local declined choice so the site does not ask again.',
