  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          "Redis: We use Redis for ephemeral data including per-user pub/sub channels for real-time event routing, cached channel data, session state tokens for OAuth flows (10-minute TTL), channel self-registration with 30-second TTL heartbeats, and, while Share usage analytics is enabled, the desktop app's live presence under its per-launch session identifier (90-second TTL). Redis data is not persisted to disk and is lost on service restart.",
