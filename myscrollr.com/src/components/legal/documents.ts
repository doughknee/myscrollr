    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          'All network communication uses HTTPS encryption. The application sends your authentication token, subscription tier, and its own version and operating system with each request; we keep the last two only as anonymous daily totals, to tell which builds a bug affects. While Share usage analytics is enabled, an authenticated daily report may say that the native ticker was visible with an enabled broad category for 30 continuous seconds. It does not transmit any data about other applications, files, browsing, symbols, teams, feeds, titles, or ticker content.',
        ],
      },
      {
      ...
      },
      {
        ...
        content: [
          'You can delete local preferences with "Reset all settings." Turning off "Share usage analytics" stops both first-party and PostHog desktop collection and deletes first-party daily facts and cohort metadata. Website PostHog opt-out stops new capture immediately and clears the browser identity. Account-linked website and desktop history uses the same server pseudonym; desktop opt-out requests deletion of that pseudonym and prior events, which PostHog processes asynchronously. Account deletion removes the local desktop event mirror immediately and retains only a deletion tombstone until that request is accepted.',
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          "Authentication, preferences, and widget data are sent only to Scrollr's API servers to authenticate requests and retrieve your personalized data. Limited PostHog app analytics are on by default and can be turned off in Data & privacy. Scrollr's API sends a pseudonymous identifier and fixed content-free activity categories to PostHog; ticker contents are never included.",
        ],
      },
      {
