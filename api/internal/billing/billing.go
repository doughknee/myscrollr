...
func HandleCreateCheckoutSession(c *fiber.Ctx) error {
	...
	var req platform.CheckoutRequest
	...
	return c.JSON(platform.CheckoutResponse{
...
func HandleCreateSetupIntent(c *fiber.Ctx) error {
	...
	return c.JSON(platform.SetupIntentResponse{
...
func HandleConfirmSubscription(c *fiber.Ctx) error {
	...
	var req platform.SubscribeRequest
	...
	platform.InvalidateOverviewCache(c.Context(), userID)

	return c.JSON(platform.SubscribeResponse{
...
func HandleCreatePaymentIntent(c *fiber.Ctx) error {
	...
	return c.JSON(platform.PaymentIntentResponse{
...
func HandleCheckoutReturn(c *fiber.Ctx) error {
	...
	return c.JSON(platform.CheckoutReturnResponse{
...
func HandleGetSubscription(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return c.JSON(platform.SubscriptionResponse{
			Plan:   "free",
			Status: "none",
		})
	}
	...
	resp := platform.SubscriptionResponse{
		Plan:             sc.Plan,
		Status:           sc.Status,
	...
	if sc.StripeSubscriptionID != nil && *sc.StripeSubscriptionID != "" {
		...
		if err != nil {
			...
			return c.JSON(platform.SubscriptionResponse{Plan: "free", Status: "none"})
		}
...
func HandleCancelSubscription(c *fiber.Ctx) error {
	...
	if sub.Status == stripe.SubscriptionStatusTrialing {
		...
		_ = accounts.RemoveUplinkRole(userID)
		_ = accounts.RemoveProRole(userID)
		_ = accounts.RemoveUltimateRole(userID)
...
func HandleChangePlan(c *fiber.Ctx) error {
	...
	var req platform.PlanChangeRequest
	if err := c.BodyParser(&req); err != nil || req.PriceID == "" {
		return c.Status(fiber.StatusBadRequest).JSON(platform.ErrorResponse{
			Status: "error", Error: "price_id is required",
		})
...
func HandlePreviewPlanChange(c *fiber.Ctx) error {
	...
	if sub.Status == stripe.SubscriptionStatusTrialing {
		return c.JSON(platform.PlanPreviewResponse{
			AmountDue:     0,
			Currency:      "usd",
			IsTrialChange: true,
			TrialEnd:      sub.TrialEnd,
