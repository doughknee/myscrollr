...
func TestNotifyTopicSubscriptionChange_PublishesControlMessage(t *testing.T) {
	...
	sub := platform.Rdb.Subscribe(ctx, platform.TopicSSEControlResubscribe)
	defer sub.Close()
	...
	case msg := <-sub.Channel():
		if msg.Channel != platform.TopicSSEControlResubscribe {
			t.Errorf("channel = %q, want %q", msg.Channel, platform.TopicSSEControlResubscribe)
		}
...
func TestHandleTopicMessage_ControlRefreshesConnectedUser(t *testing.T) {
	...
	staleTopic := platform.TopicPrefixFinance + "STALE"
	h.registry.subscribe(userID, staleTopic)

	h.handleTopicMessage(platform.TopicSSEControlResubscribe, []byte(userID))
...
func TestHandleTopicMessage_ControlNoConnection_NoOp(t *testing.T) {
	...
	topic := platform.TopicPrefixSports + "NFL"
...
func TestHandleTopicMessage_CoreUserTopicStillDispatches(t *testing.T) {
	...
	h.handleTopicMessage(platform.TopicPrefixCore+userID, []byte(`{"kind":"prefs"}`))
