...
func TestHubStress(t *testing.T) {
	...
	h := swapHub(t)
	h.dispatchCh = make(chan dispatchJob, platform.SSEDispatchQueueSize)
	h.invalCh = make(chan string, platform.SSEInvalidationQueueSize)
	...
	for u := 0; u < users; u++ {
		...
		go func(u int) {
			...
			for c := 0; c < clientsPerUser; c++ {
				cl := &Client{UserID: uid, Ch: make(chan []byte, platform.SSEClientBufferSize)}
				h.register(cl)
				clientsMu.Lock()
	...
	allowance := 2*platform.Rdb.Options().PoolSize + 32
