...
// currentDesktopVersion reads the version out of the knowledge base, which
// generates it from desktop/package.json and is regenerated by CI. One
// source, already embedded — no second copy to drift.
var (
	currentVersionOnce sync.Once
	currentVersion     string
	currentVersionRe   = regexp.MustCompile(`Current desktop version: \*\*([^*]+)\*\*`)
)

func currentDesktopVersion() string {
	currentVersionOnce.Do(func() {
		if m := currentVersionRe.FindStringSubmatch(supportKnowledgeBase()); len(m) == 2 {
			currentVersion = m[1]
		} else {
