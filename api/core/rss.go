...
func RegisterRSSRoutes(app *fiber.App) {
	app.Get("/rss/feeds", LogtoAuth, handleGetRSSFeedCatalog)
	app.Delete("/rss/feeds", LogtoAuth, handleDeleteCustomFeed)
...
func queryUserCatalog(ctx context.Context, userSub string, includeFailing bool) ([]TrackedFeed, error) {
	...
	rows, qErr := DBPool.Query(ctx, query, userSub, MaxConsecutiveFailures)
	if qErr != nil {
		return nil, qErr
	}
	defer rows.Close()
...
func invalidateUserCatalogCache(ctx context.Context, userSub string) {
	...
	Rdb.Del(ctx, CacheKeyRSSCatalog+":"+userSub)
	Rdb.Del(ctx, CacheKeyRSSCatalog+":"+userSub+":all")
}
...
func rssLifecycle(event, userSub string, config, oldConfig map[string]interface{}, enabled bool) {
	...
	case "updated":
		...
		Rdb.Del(ctx, CacheKeyRSSPrefix+userSub)
		go syncRSSFeedsToTracked(userSub, config)
	case "deleted":
		Rdb.Del(ctx, CacheKeyRSSPrefix+userSub)
	case "sync":
...
func getUserRSSFeedURLs(ctx context.Context, logtoSub string) []string {
	...
	for rows.Next() {
		...
		for _, u := range extractFeedURLsFromConfig(config) {
			if !seen[u] {
				seen[u] = true
				urls = append(urls, u)
			}
...
func syncRSSFeedsToTracked(userSub string, config map[string]interface{}) {
	...
	curatedRows, curErr := DBPool.Query(ctx, `SELECT url FROM tracked_feeds WHERE is_default = true`)
	if curErr != nil {
...
func runJanitorOnce(parent context.Context) {
	...
	locked, err := Rdb.SetNX(ctx, janitorLockKey, "1", janitorLockTTL).Result()
	if err != nil {
...
func disableBrokenCuratedFeeds(ctx context.Context) (int, error) {
	...
	`
	cmd, err := DBPool.Exec(ctx, q, MaxConsecutiveFailuresJanitor)
	if err != nil {
		return 0, err
	}
...
func removeBrokenCustomFeeds(ctx context.Context) (int, error) {
	...
	`
	rows, err := DBPool.Query(ctx, findQ, MaxConsecutiveFailuresJanitor)
	if err != nil {
		return 0, err
	}
	urls := make([]string, 0)
