...
func RegisterPredictionsRoutes(app *fiber.App) {
	app.Get("/predictions", LogtoAuth, handleGetPredictions)
	...
	app.Get("/predictions/candlesticks/:ticker", LogtoAuth, handleGetCandlesticks)
}
...
func handleGetCandlesticks(c *fiber.Ctx) error {
	...
	if err := Rdb.Set(ctx, cacheKey, string(body), CandlesticksCacheTTL).Err(); err != nil {
...
func invalidatePredictionsUserCache(userSub string) {
	if err := Rdb.Del(context.Background(), CacheKeyPredictionsPrefix+userSub).Err(); err != nil {
...
func queryMarkets(ctx context.Context) ([]Prediction, error) {
	rows, err := DBPool.Query(ctx, MarketsQuery)
	if err != nil {
