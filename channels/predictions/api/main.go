...
const (
	...
	RegistrationKey = "channel:predictions"
	...
	DefaultChannelURL = "http://localhost:8085"
)
...
func main() {
	...
	fiberApp := fiber.New(fiber.Config{
		AppName:               "Scrollr Predictions API",
	...
	fiberApp.Get("/predictions", app.getPredictions)
	...
	log.Printf("Predictions API listening on port %s", port)
	...
	log.Println("Shutting down Predictions API...")
	cancel()
...
func startRegistration(ctx context.Context, rdb *redis.Client) {
	...
	payload := registrationPayload{
		...
		CDCTables:    []string{"markets"},
		Routes: []registrationRoute{
			{Method: "GET", Path: "/predictions", Auth: true},
