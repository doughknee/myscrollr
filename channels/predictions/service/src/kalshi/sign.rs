...
mod tests {
    ...
    fn loads_pkcs1_pem_and_signs() {
        ...
        let (ts, sig) = signer.sign("GET", "/trade-api/v2/markets").expect("sign");
    ...
    fn signatures_are_randomized() {
        ...
        let (_, a) = signer.sign("GET", "/trade-api/v2/markets").unwrap();
        let (_, b) = signer.sign("GET", "/trade-api/v2/markets").unwrap();
